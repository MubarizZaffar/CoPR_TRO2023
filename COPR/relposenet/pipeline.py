...
from relposenet.dataset import SevenScenesRelPoseDataset_MZ
...
class Pipeline(object):
    def __init__(self, cfg):
        ...
        self.writer = SummaryWriter(self.cfg.output_params.logger_dir+datetime.now().strftime('%b%d_%H-%M-%S') \
                                    +'8linearlayers_resnet34_originalmodel_ds507scenes_5e_05_stationescalator_'+self.cfg.data_params.datasetname+'_'+str(self.cfg.train_params.lr)+str(self.cfg.train_params.scheduler.lrate_decay_steps))
    ...
    def _init_dataloaders(self):
        ...
        train_dataset = SevenScenesRelPoseDataset_MZ(cfg=self.cfg, split='train', transforms=train_augs)

        val_dataset = SevenScenesRelPoseDataset_MZ(cfg=self.cfg, split='val', transforms=val_augs)
    ...
    def _predict_cam_pose(self, mini_batch):
        # feat1, feat2, q_est, t_est = self.model.forward(mini_batch['img1'].to(self.device),
        #                                   mini_batch['img2'].to(self.device))
        
        feat2 = self.model.forward(mini_batch['feat1'].to(self.device),
                                  mini_batch['RT'].to(self.device))
        return feat2#q_est, t_est

    def _save_model(self, step, loss_val, best_val=False):
        #resnet34_originalmodel_newest_mixed_7scenes_5e_06_ or resnet34_originalmodel_newest_mixed_University_1e_05_ 
        runid='8linearlayers_resnet34_originalmodel_ds507scenes_5e_05_stationescalator_'\
                                    +self.cfg.data_params.datasetname+'_'+str(self.cfg.train_params.lr)+str(self.cfg.train_params.scheduler.lrate_decay_steps)
        if not osp.exists(self.cfg.output_params.snapshot_dir+'descregressor'+runid):
            os.makedirs(self.cfg.output_params.snapshot_dir+'descregressor'+runid)

        fname_out = 'best_val.pth' if best_val else 'snapshot{:06d}.pth'.format(step)
        save_path = osp.join(self.cfg.output_params.snapshot_dir+'descregressor'+runid, fname_out)
    ...
    def _train_batch(self):
        ...
        feat2 = self._predict_cam_pose(train_sample)
        ...
        loss = self.criterion(train_sample['feat2_gt'].to(self.device),
                                                      feat2)
        loss.backward()
    ...
    def _validate(self):
        ...
        with torch.no_grad():
            for val_sample in tqdm(self.val_loader):
                feat2 = self._predict_cam_pose(val_sample)
                # compute loss
                loss = self.criterion(val_sample['feat2_gt'].to(self.device),
                                                      feat2)
